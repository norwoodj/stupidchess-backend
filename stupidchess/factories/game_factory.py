...
def create_new_game(game_type, game_auth_type, other_player):
    ...
    other_player_obj = current_app.context.user_service.get_user_with_username(
        other_player
    )

    if other_player_obj is None:
        raise InvalidGameParameterException(
            "No Player with username '{}' exists".format(other_player)
        )
...
STUPID_CHESS_INITIAL_PIECES_TO_BE_PLACED = [
    ...
    for idx, piece_type in enumerate(STUPID_CHESS_INTIAL_PIECES_FOR_SIDE)
    for color in (Color.BLACK, Color.WHITE)
]


def get_new_stupid_chess_game(
    black_player_uuid, white_player_uuid, black_player_name, white_player_name
):
    return Game(
...
CHESS_INITIAL_PIECES = [
    ...
    *[
        Piece(type=PieceType.PAWN, color=Color.BLACK, square=square)
        for square in range(10, 18)
    ],
    ...
    *[
        Piece(type=PieceType.PAWN, color=Color.WHITE, square=square)
        for square in range(60, 68)
    ],
]
...
CHECKERS_INITIAL_PIECES = [
    *[
        Piece(type=PieceType.CHECKER, color=Color.BLACK, square=square)
        for square in range(0, 8, 2)
    ],
    *[
        Piece(type=PieceType.CHECKER, color=Color.BLACK, square=square)
        for square in range(11, 18, 2)
    ],
    *[
        Piece(type=PieceType.CHECKER, color=Color.BLACK, square=square)
        for square in range(20, 28, 2)
    ],
    *[
        Piece(type=PieceType.CHECKER, color=Color.WHITE, square=square)
        for square in range(51, 58, 2)
    ],
    *[
        Piece(type=PieceType.CHECKER, color=Color.WHITE, square=square)
        for square in range(60, 68, 2)
    ],
    *[
        Piece(type=PieceType.CHECKER, color=Color.WHITE, square=square)
        for square in range(71, 78, 2)
    ],
]
