...
class MoveApplicationService:
    def __init__(self, game_service, move_service, move_update_services):
        ...
        self.__move_update_service_by_type = {
            ms.get_move_type(): ms for ms in move_update_services
        }
    ...
    def __is_player_authorized_to_perform_move(game, user_uuid, move):
        color = (
            ...
            else MoveApplicationService.__color_for_move_move(game, move)
        )
        move_player_uuid = (
            game.blackPlayerUuid if color == Color.BLACK else game.whitePlayerUuid
        )
    ...
    def apply_move(self, game_uuid, user_uuid, move):
        ...
        if not MoveApplicationService.__is_player_authorized_to_perform_move(
            game, user_uuid, move
        ):
        ...
        LOGGER.debug(
            f"Applying move to game {game.get_id()} at lastMove {game.lastMove}: {move}"
        )
        ...
        if len(moves_to_apply) > 1:
            LOGGER.debug(
                f"Will apply additional move(s) to game {game.get_id()}: {', '.join(str(m) for m in moves_to_apply[1:])}"
            )
        ...
        try:
            self.__move_service.insert_many(
                [
                    self.__move_update_service_by_type[m.type].get_move_for_insert(m)
        ...
        LOGGER.debug(
            f"Successfully saved move indexes {', '.join(str(m.index) for m in moves_to_apply)} for game {game.get_id()}"
        )
        ...
        for move_type, moves_for_type in groupby(moves_to_apply, key=lambda m: m.type):
            ...
            LOGGER.debug(
                f"Applying game updates for move type {move_type} to game {game.get_id()} for moves {', '.join(str(m) for m in moves_for_type)}"
            )
            self.__move_update_service_by_type[move_type].apply_game_updates_for_moves(
                moves_for_type, game
            )
